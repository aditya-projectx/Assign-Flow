...
export const getHodDashboard = async (req, res) => {
  try {
    ...
    const reviewed = await Assignment.countDocuments({
      ...query,
...
export const getHodDetails = async (req, res) => {
  try {
    ...
    res.render("hod/details", { assignment: assignmentData, user: req.user });
  } catch (err) {
...
export const hodViewAssignmentFile = async (req, res) => {
  try {
    const assignment = await Assignment.findById(req.params.id);
    ...
    console.error("HOD View File Error:", err);
    res.status(500).send("Error opening file");
  }
};

export const hodDownloadAssignmentFile = async (req, res) => {
  try {
    const assignment = await Assignment.findById(req.params.id);
    if (!assignment) return res.status(404).send("File not found");
    ...
    console.error("HOD Download File Error:", err);
