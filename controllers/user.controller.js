...
export const updateUser = async (req, res) => {
  try {
    ...
    // Server-side email validation for updates
    if (!isValidEmail(email)) {
      const user = await User.findById(userId).select("-password").lean();
      const departments = await Department.find().select("name").lean();
      return res.render("admin/edit-user", {
...
export const deleteUser = async (req, res) => {
  try {
    ...
    // CHECK: Prevent deletion if email is invalid
    if (!isValidEmail(user.email)) {
      return res.redirect("/admin/users?error=Cannot delete user with invalid email format: " + user.email);
    }

    // Send deletion email BEFORE deleting (now safe)
    await sendEmail({
      to: user.email,
