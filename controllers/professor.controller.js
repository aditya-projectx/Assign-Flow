...
export const initiateReview = async (req, res) => {
  try {
    ...
    const signatureUpload = req.file
      ? await uploadBufferToCloudinary(req.file, {
          folder: "assignflow/signatures",
          resource_type: "image",
        })
      : null;
    const signatureImage = signatureUpload ? signatureUpload.secure_url : null;
    ...
    await sendEmail({
      to: user.email,
      subject: "AssignFlow: Verify Assignment Review",
...
export const professorViewAssignmentFile = async (req, res) => {
  try {
    const assignment = await Assignment.findById(req.params.id);
    ...
    const reviewerMatches =
      ...
      assignment.history?.some((entry) => entry.reviewerId?.toString() === req.user.id);

    if (!reviewerMatches) {
      return res.status(403).send("Unauthorized");
    }

    if (assignment.fileUrl?.startsWith("http")) {
    ...
    console.error("Professor View File Error:", err);
    res.status(500).send("Error opening file");
  }
};

export const professorDownloadAssignmentFile = async (req, res) => {
  try {
    const assignment = await Assignment.findById(req.params.id);
    if (!assignment) return res.status(404).send("File not found");
    ...
    return res.download(assignment.fileUrl);
  } catch (err) {
    console.error("Professor Download File Error:", err);
